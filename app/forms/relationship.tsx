     ...
     function Options(props:{data : any})
{
    ...
    return(<View style={{height:"100%"}}><Pressable onPress={() => router.navigate('/signup')} ><SvgComponent width={"75px"} style={{marginLeft:10}}></SvgComponent></Pressable><SafeAreaView>
        <View>
            <Text style={styles.text1}>What's your relationship status?</Text>
            <ToggleGroup onValueChange={(e) => setData(e)} unstyled type="single" style={{flexWrap:"wrap", width:410, marginLeft:12, marginTop:-20}}>
            {options.map((data) => <ToggleGroup.Item unstyled value={data}><Options data={data}></Options></ToggleGroup.Item>)}
            </ToggleGroup>
        </View>

        <Button unstyled style={{width:50, padding:0, backgroundColor:"transparent", color:"#7f5539", position:"absolute", bottom: -190, left:310}}><Button.Text>Skip</Button.Text></Button>
       
    </SafeAreaView><Button style={styles.continuebut} onPress={() => router.push('/forms/creating')}><Button.Text style={styles.buttxt}>Continue</Button.Text></Button></View>)
}